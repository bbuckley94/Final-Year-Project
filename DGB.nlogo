breed [points point]
breed [radii radius]
undirected-link-breed[laps lap]
directed-link-breed[r rs]
;breed [walls wall]
points-own [goal static?]
patches-own [visited?]
globals [pcurrent rcurrent far free li]

to setup
  clear-all

  set-default-shape points "circle"
  set-default-shape radii "circle"
  set pcurrent 0
  set rcurrent 1
  ...
  ask points [
    ...
    set color green
    set goal max-one-of patches [distance myself]
    show max-one-of patches [distance myself]
    set static? false
    make-radius
  ]
  ...
  ask patches [
    set visited? false
    ;set free patch-set self
    ;map patch
    ;[set free [pxcor pycor]]
  ]
...
to go
  ;if (count (points with [goal != []]) = 0) [stop]
  if (count points >= population) [stop]
  ...
  ask point pcurrent [
    ...
    [if any? lap-neighbors = false or li > 5[fd 0.1]
  ...
  ;;if patch visited don't allow goal

  tick
end

to spawn
  set pcurrent pcurrent + 2
  ;set free free - vis
   ...
   hatch 1 [
      ...
      set color green
      set static? false
      ask points with [self < myself] [create-lap-with myself]
      ;set far max-one-of patches [distance myself]

      ;ifelse is-patch? far visited? = false
      ;[set goal far]
      ;[set goal max-one-of patches [distance myself + 3]]
